...
public class ImgDwnUISinglePage {
    static File dir;
    ...
    public static void getImgLinks() throws InterruptedException, IOException {
        initializeDriver();
        navigateToPage("https://vipergirls.to/threads/1111573-Galitsin-Works-A-Z-(1999-2019)?p=47698574&viewfull=1#post47698574");
    ...
    private static void processContent(WebElement content) throws InterruptedException, IOException {
    	List<WebElement> titleEl = content.findElements(By.cssSelector("font[size='3']"));
    	String title = "";
    	if(titleEl.size() > 0) {
        ...
        if (imgLinks.size() > 5 && titleEl.size() > 0) {
    ...
    private static void processImageLinks(List<WebElement> imgLinks, String title) throws InterruptedException, IOException {
        ...
        for (WebElement imgLink : imgLinks) {
            ...
            if(url.contains("https://thumbs2.imgbox")) {
